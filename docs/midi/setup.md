...
does not turn incoming notes or transport messages on and off. iOS chooses
both routes in the app's MIDI settings instead.
...
4. For output, leave `MIDI DEVICE` on any non-`OFF` value.
